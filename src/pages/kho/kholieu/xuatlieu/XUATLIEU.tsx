    ...
    await generalQuery("checkMNAMEfromLotI222XuatKho", { M_LOT_NO: M_LOT_NO, PLAN_ID: PLAN_ID })
      .then((response) => {
        if (response.data.tk_status !== "NG") {
          ...
          let dkxl_exist: boolean = dangkyxuatlieutable.filter((ele: DKXL_DATA, index: number) => {
          ...
          let OUT_DATE: string = dkxl_exist ? lot_info.OUT_DATE : 'NG';
          let OUT_NO: string = dkxl_exist ? lot_info.OUT_NO : "NG";
          let OUT_SEQ: string = dkxl_exist ? lot_info.OUT_SEQ : "NG";
