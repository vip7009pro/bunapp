...
const INSPECT_REPORT = () => {
  const [dailyppm1, setDailyPPM1] = useState<PPMData[]>([]);
  const [weeklyppm1, setWeeklyPPM1] = useState<PPMData[]>([]);
  const [monthlyppm1, setMonthlyPPM1] = useState<PPMData[]>([]);
  const [yearlyppm1, setYearlyPPM1] = useState<PPMData[]>([]);
  const [dailyppm2, setDailyPPM2] = useState<PPMData[]>([]);
  const [weeklyppm2, setWeeklyPPM2] = useState<PPMData[]>([]);
  const [monthlyppm2, setMonthlyPPM2] = useState<PPMData[]>([]);
  const [yearlyppm2, setYearlyPPM2] = useState<PPMData[]>([]);
  const [dailyppm, setDailyPPM] = useState<PPMData[]>([]);
  const [weeklyppm, setWeeklyPPM] = useState<PPMData[]>([]);
  const [monthlyppm, setMonthlyPPM] = useState<PPMData[]>([]);
  const [yearlyppm, setYearlyPPM] = useState<PPMData[]>([]);
  ...
  const handle_getWeeklyPPM = async (FACTORY: string, listCode: string[]) => {
    ...
    await generalQuery("inspect_weekly_ppm", {
      ...
      .then((response) => {
        //console.log(response.data.data);
        if (response.data.tk_status !== "NG") {
          const loadeddata: PPMData[] = response.data.data.map(
            (element: PPMData, index: number) => {
              return {
                ...element,
                TOTAL_PPM: ng_type === "ALL" ? element.TOTAL_PPM : ng_type === "P" ? element.PROCESS_PPM : element.MATERIAL_PPM,
                MATERIAL_PPM: ng_type === "ALL" ? element.MATERIAL_PPM : ng_type === "P" ? 0 : element.MATERIAL_PPM,
